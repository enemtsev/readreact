...
TEST_F(ReadClassTest, Initialization) {
    ...
    EXPECT_CALL(*gpioMock, pin_configure(&dummy_dev, 0, GPIO_INPUT | GPIO_PULL_UP)).WillOnce(Return(0));
    EXPECT_CALL(*gpioMock, pin_interrupt_configure(&dummy_dev, 0, GPIO_INT_EDGE_BOTH)).WillOnce(Return(0));
    EXPECT_CALL(*gpioMock, add_callback(&dummy_dev, _)).WillOnce(Return(0));
...
TEST_F(ReadClassTest, InterruptHandling) {
    ...
    EXPECT_CALL(*gpioMock, pin_configure(&dummy_dev, 0, GPIO_INPUT | GPIO_PULL_UP)).WillOnce(Return(0));
    EXPECT_CALL(*gpioMock, pin_interrupt_configure(&dummy_dev, 0, GPIO_INT_EDGE_BOTH)).WillOnce(Return(0));
    EXPECT_CALL(*gpioMock, add_callback(&dummy_dev, _)).WillOnce(DoAll(SaveArg<1>(&saved_cb), Return(0)));

    ReadClass reader("input-gpio");

    // Simulate interrupt
    EXPECT_CALL(*gpioMock, pin_get(&dummy_dev, 0)).WillOnce(Return(1));  // High state
